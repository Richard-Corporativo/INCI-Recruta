<!-- @c architecture | doc | (nÃ£o-apagar)-gerador-de-clone-de-site-com-ia | @v 0.1.0 | @> 
<!-- @. resumo | <deps | >users
# Arquitetura do Projeto: Engenharia de Software AI
...
O sistema de **Engenharia de Software AI** é um agente projetado para auxiliar em tarefas de desenvolvimento e arquitetura de software, operando localmente no desktop do usuário. Sua interface primária de controle é via Terminal ou API local, permitindo uma interação fluida via texto, documentos e código. O sistema é construído para ser modular, extensível através de "skills" (habilidades) e totalmente focado na produtividade do desenvolvedor, mantendo a persistência de dados localmente.

A arquitetura segue um fluxo de pipeline onde os comandos de engenharia são capturados, processados por um motor de raciocínio (Agent Loop) que utiliza LLMs externos (como Gemini ou DeepSeek) apenas para inferência, e responde ao desenvolvedor com soluções técnicas, podendo inclusive gerar arquivos de código ou diagramas.
...
| Interface de Comando | Funcional | Alta | Interface via CLI ou API REST local. |
...
| Segurança de Acesso | Funcional | Crítica | Controle de acesso baseado em autenticação local. |
...
graph TB
    User([👤 Desenvolvedor])
    Interface[💻 CLI / IDE Plugin]
    System[🏗️ Software Eng Engine]
    LLM[🧠 LLM APIs - Gemini/DeepSeek]
    Whisper[🎙️ Whisper Local - STT]
    EdgeTTS[🔊 Edge-TTS - TTS]

    User <-->|Envia Comandos/Código| Interface
    Interface <-->|Processa/Executa| System
...
graph TB
    subgraph "Camada de Interface (Input/Output)"
        InputH[CommandInputHandler]
        OutputH[CommandOutputHandler]
    end
...
| **Interface** | **Node.js CLI / Express** | Interface flexível para integração com IDEs e terminal. |
...
5.  **Strategy:** No `CommandOutputHandler` para decidir entre enviar texto puro, chunks ou arquivos de código.
...
sequenceDiagram
    ...
    participant I as CommandInputHandler
    ...
    U->>I: Envia Comando/Código
    I->>I: Valida Autenticação
...
| Rate Limiting API | Inferido | Manter a lógica de `sleep` no OutputHandler caso a API de LLM retorne erro 429. |
