<!-- @c agent-loop | doc | (nÃ£o-apagar)-gerador-de-clone-de-site-com-ia | @v 0.1.0 | @> 
<!-- @. resumo | <deps | >users
# Spec: Agent Loop (Reasoning Engine for Software Engineering)
...
O **Agent Loop** é a engrenagem central do Engenharia de Software AI. Ele implementa o padrão ReAct (Reasoning and Acting). É o módulo onde uma tarefa de engenharia entra, é submetida ao LLM base (Thought), uma ou mais ferramentas de desenvolvimento são chamadas (Action+Observation), até um veredito de resposta técnica ser chegado, repetindo em loop limitado de iterações para evitar impasses de contexto infinito.
...
**Por que agora:**
Precisamos desacoplar a parte de interface/entrada de dados da parte de processamento puramente sistêmico (Tool calls/Registry).
...
**Módulo Cliente Primário:** Classes dentro de `CommandController` ou `SkillExecutor` que invocam o AgentLoop repassando o array de mensagens antigas e o System Prompt correspondente às ferramentas em registro ativo.
