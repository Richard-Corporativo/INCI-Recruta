<!-- @c memory | doc | (nÃ£o-apagar)-gerador-de-clone-de-site-com-ia | @v 0.1.0 | @> 
<!-- @. resumo | <deps | >users
...
O módulo de persistência de estado do Engenharia de Software AI gerencia tanto o histórico de desenvolvimento em banco de dados SQLite (`better-sqlite3`) quanto atua como manager da janela de contexto para impedir que o limite máximo do envelope de tokens da IA (Context Window) estoure.
...
- [ ] G-01: Prover Storage fixo e rápido de comandos e respostas para recriar o contexto de engenharia.
...
- A Classe `CommandController` via `AgentLoop` (lê e grava respostas).
...
| RF-02 | O Storage deve usar WAL (Write-Ahead Logging) ativo pra manter leitura sem block | Must | Múltiplas requisições simultâneas não congelam o sistema por locks do sqlite3 nativo. |
...
| EC-02: Null Bytes na Entrada do Usuário | Receber bytes invisíveis causando Erro de syntax DB. | Stripping na entrada `content.replace(/\u0000/g, '')`. O DB não engole a query suja. |
...
- **Arquivos DB Sensíveis:** O `soft_eng.db` jamais pode ir pro Git (Adicionar no `.gitignore` /data).
