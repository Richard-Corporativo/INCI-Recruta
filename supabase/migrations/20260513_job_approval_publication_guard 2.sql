...
BEGIN
  ...
  IF (TG_OP = 'INSERT' OR NEW.workflow_status IS DISTINCT FROM OLD.workflow_status) AND NEW.workflow_status = 'published' THEN
    NEW.status := 'Ativa';
    NEW.approval_status := 'Aprovado';
  ELSIF (TG_OP = 'INSERT' OR NEW.workflow_status IS DISTINCT FROM OLD.workflow_status) AND NEW.workflow_status = 'approved' THEN
    NEW.approval_status := 'Aprovado';
  ELSIF (TG_OP = 'INSERT' OR NEW.workflow_status IS DISTINCT FROM OLD.workflow_status) AND NEW.workflow_status = 'pending_approval' THEN
    NEW.approval_status := 'Pendente';
  ELSIF (TG_OP = 'INSERT' OR NEW.workflow_status IS DISTINCT FROM OLD.workflow_status) AND NEW.workflow_status = 'draft' THEN
    NEW.status := 'Rascunho';
