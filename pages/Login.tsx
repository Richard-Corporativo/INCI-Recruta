...
const Login: React.FC = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    const validUsers = [
      { email: 'israel.richard@incibrasil.com.br', pass: 'incibrasil1234' },
      { email: 'balab.incibrasil@gmail.com', pass: 'balab2025' }
    ];
    ...
    if (isValid) {
      ...
      const token = 'mock_token_' + new Date().getTime(); // Simulação de token
      
      if (keepConnected) {
        localStorage.setItem('recruitSys_token', token);
      } else {
        sessionStorage.setItem('recruitSys_token', token);
      }
