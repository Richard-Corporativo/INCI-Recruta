            ...
            <main className="flex-1 flex flex-col items-center justify-center p-4 sm:p-6 relative z-10 w-full max-w-7xl mx-auto">
                <div className="w-full max-w-[440px] bg-card border border-border rounded-lg shadow-xl overflow-hidden">
                    <div className="bg-primary/5 px-6 py-3 border-b border-border flex items-center justify-center gap-2">
                        ...
                        <span className="text-xs font-bold text-primary uppercase tracking-wide">Ambiente Seguro</span>
                    </div>
                    <div className="p-8">
                        <div className="text-center mb-8">
                            ...
                            <h1 className="text-xl font-bold text-foreground mb-2">Verificação em Duas Etapas</h1>
                            <p className="text-sm text-muted-foreground leading-relaxed font-medium">
                                Detectamos que seu perfil possui privilégios de <span className="font-bold text-foreground">Admin/Qualidade</span>.
                            </p>
                        ...
                        <form action="#" className="flex flex-col gap-6" method="POST">
                            <div className="space-y-3">
                                <label className="block text-sm font-bold text-foreground" htmlFor="verification-code">
                                    Código de verificação
                                </label>
                                <div className="relative group">
                                    <input autoComplete="one-time-code" autoFocus className="tracking-[0.75em] text-center w-full bg-background border border-border rounded-base py-3 px-4 text-2xl font-bold text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:border-ring transition-all duration-200 ease-in-out shadow-inner" id="verification-code" maxLength={6} name="code" placeholder="• • • • • •" type="text" />
                            ...
                            <div className="flex items-center justify-between py-1">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-xs font-bold text-muted-foreground">Expira em 04:59</span>
                                </div>
                                <button className="text-xs font-bold text-primary hover:text-primary/80 transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-1 rounded" type="button">
                                    Reenviar código
                                </button>
                            </div>
                            <button className="w-full flex items-center justify-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-3 px-4 rounded-base shadow-lg shadow-primary/20 transition-all duration-200 ease-in-out active:translate-y-[1px]" type="button">
                                <span className="material-symbols-outlined text-[20px]">check_circle</span>
                                Validar Acesso
                            </button>
                        </form>
                        <div className="mt-8 pt-6 border-t border-border text-center">
                            <Link to="/login" className="inline-flex items-center gap-2 text-sm font-bold text-muted-foreground hover:text-foreground transition-all duration-200 ease-in-out">
