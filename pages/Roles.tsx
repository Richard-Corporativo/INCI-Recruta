...
const Roles: React.FC = () => {
  ...
  const filteredRoles = roles
    .map(role => ({
      ...role,
      activeJobsCount: jobs.filter(j => j.role_id === role.id && j.status === 'Ativa').length
    }))
    .filter(role => {
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-background transition-colors duration-200">
      ...
      <header className="bg-card border-b border-border pt-8 pb-4 px-8 z-20 shadow-sm shrink-0 sticky top-0">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-semibold text-foreground tracking-tight">Biblioteca de cargos</h1>
      ...
      <main className="flex-1 overflow-y-auto bg-muted/30 p-8">
        <div className="max-w-[1920px] mx-auto pb-12">
          {/* Filters */}
          <div className="bg-card p-5 rounded-lg border border-border shadow-sm mb-6 max-w-4xl transition-all">
              ...
              <div className="md:col-span-2">
                ...
                <div className="relative mt-1.5">
                  ...
                  <input
                    ...
                    placeholder="Nome do cargo, departamento ou código..."
                    type="text"
                    value={searchTerm}
          ...
          {/* Table */}
          <div className="bg-card rounded-lg border border-border shadow-sm overflow-hidden transition-all">
              ...
              <tbody className="divide-y divide-border">
                {filteredRoles.map((role) => (
                  <tr key={role.id} className="group hover:bg-muted/40 transition-all duration-200 ease-in-out">
                    <td className="px-6 py-5 font-medium transition-all">
                      <div className="flex flex-col">
                        <span className="text-sm font-semibold text-foreground leading-tight tracking-tight">{role.title}</span>
                        <div className="flex items-center gap-2 mt-1.5">
                          <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider px-1.5 py-0.5 bg-muted rounded border border-border/50">{role.code}</span>
                          {role.revision_code && (
                            <span className="text-[9px] text-muted-foreground font-medium italic">rev. {role.revision_code}</span>
                          )}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <span className="inline-flex items-center px-2.5 py-1 rounded bg-muted/60 text-foreground border border-border/50 text-xs font-semibold transition-all">
                        {role.department}
                      </span>
                    </td>
                    <td className="px-6 py-5 text-center">
                      <span className={`text-sm font-bold ${role.activeJobsCount > 0 ? 'text-primary' : 'text-muted-foreground'} transition-colors`}>{role.activeJobsCount || 0}</span>
                    </td>
                    <td className="px-6 py-5 transition-all">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-semibold transition-all ${role.status === 'Ativo' ? 'bg-emerald-500/10 text-emerald-600 border border-emerald-500/20' : 'bg-muted text-muted-foreground border border-border'}`}>
                        <span className={`size-1.5 rounded-full ${role.status === 'Ativo' ? 'bg-emerald-500 animate-pulse-slow' : 'bg-muted-foreground'}`}></span>
                    ...
                    <td className="px-6 py-5 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          onClick={() => openQuickView('role', role)}
                          className="flex items-center justify-center size-9 text-muted-foreground hover:text-primary hover:bg-primary/10 rounded-base transition-all duration-200"
                        ...
                        {!isManager && (
                          <>
                            ...
                            <button
                              ...
                              className="flex items-center justify-center size-9 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-base transition-all duration-200"
                              title="Excluir Cargo"
            ...
            <div className="px-6 py-4 border-t border-border bg-muted/20">
              <span className="text-xs text-muted-foreground font-semibold italic">
                {filteredRoles.length === 0 ? 'Nenhum cargo encontrado' : `Mostrando ${filteredRoles.length} cargos na biblioteca`}
              </span>
            </div>
          </div>
        </div>
