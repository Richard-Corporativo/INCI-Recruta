...
[2026-06-02] Atualização dos Termos de Uso baseada na revisão jurídica: (1) Atualizadas as 20 seções em TermsModal.tsx. (2) Atualizadas as 20 seções e data de versão (02 de Junho de 2026) em TermsOfUse.tsx. (3) Compilação validada com npx tsc --noEmit. (4) Reduzido o espaçamento vertical das listas (de space-y-2 para space-y-1).
...
[2026-04-29] Fix Build Error: Corrigido alias @/* no tsconfig.json e integrados layouts (Candidate/Public) nativamente no App Router (Next.js 15).
...
[2026-04-29] docs/SYSTEM*MAP.md atualizado com mapeamento exaustivo de ~130 arquivos fonte (5 services, 9 hooks, 2 contexts, 6 libs, ~65 componentes, 27 rotas).
...
[2026-04-29] Normalização de Imports: Substituídos caminhos relativos (`../`) por aliases canônicos (`@src/*`) em todos os componentes de `src/components/admin/`e`src/pages/`.
...
Senha candidato divergente: DONE | israel.richard@incibrasil.com.br — senha atualizada para 'incibrasil1234' via SQL (crypt/bcrypt, verificado).
