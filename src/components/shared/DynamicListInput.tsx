// @component DynamicListInput | @tipo componente | @versao 1.0.0
// > Input de lista dinâmica genérico — add/remove items com Enter
// @api items: string[], onChange: fn, placeholder?: string, label?: string
    ...
    return (
        <div className="flex flex-col gap-3" ref={wrapperRef}>
            <label className="text-[11px] font-semibold text-muted-foreground flex items-center gap-2">
                {icon && <Icon icon="material-symbols:{icon}" className="text-[16px]" width="16" height="16" />}
                {label}
            </label>
            ...
            <div className="relative">
                ...
                {showOptions && filteredOptions.length > 0 && (
                    <div className="absolute top-full left-0 w-full mt-1 bg-card rounded-2xl z-50 max-h-48 overflow-y-auto custom-scrollbar animate-in fade-in zoom-in-95 duration-200 p-6">
                        {filteredOptions.map((option, idx) => (
                            ...
                            >
                                <Icon icon="material-symbols:add" className="text-muted-foreground h-5 w-5" aria-hidden="true" />
                                {option}
                            </button>
                        ))}
                    </div>
