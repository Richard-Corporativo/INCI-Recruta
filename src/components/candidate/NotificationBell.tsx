...
export function NotificationBell() {
    ...
    return (
        <div ref={ref} className="relative">
            ...
            {open && (
                <div className="absolute right-0 top-full z-[80] mt-2 w-80 overflow-hidden rounded-2xl border border-border bg-card shadow-none animate-in fade-in zoom-in-95">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-border">
                        <span className="text-xs font-semibold text-foreground uppercase tracking-[0.15em]">Notificações</span>
                        {unreadCount > 0 && (
                            <button
                                onClick={markAllRead}
                    ...
                    <div className="max-h-[360px] overflow-y-auto">
                        {isLoading ? (
                            ...
                            <p className="px-4 py-6 text-center text-xs text-muted-foreground">Nenhuma notificação</p>
                        ) : (
                            notifications.map(n => (
                                <button
                                    key={n.id}
                                ...
                                >
                                    ...
                                    <span className="text-[11px] text-muted-foreground leading-snug line-clamp-2">{n.message}</span>
                                    <div className="flex items-center gap-2 mt-0.5">
                                        <span className="text-[10px] text-muted-foreground/70">{TYPE_LABEL[n.type] ?? n.type}</span>
