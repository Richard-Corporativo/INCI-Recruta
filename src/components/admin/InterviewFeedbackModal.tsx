// @component InterviewFeedbackModal | @tipo componente | @versao 1.0.0
// > Modal para registrar feedback de entrevista — rating, strengths, concerns
// @api candidateId: string, isOpen: bool, onClose: fn, onSubmit: fn
...
const InterviewFeedbackModal: React.FC<InterviewFeedbackModalProps> = ({
      ...
      if (currentStage) {
        ...
        if (currentColumn) {
          ...
          if (currentIndex !== -1 && currentIndex < COLUMNS_CONFIG.length - 1) {
            setSelectedStage(COLUMNS_CONFIG[currentIndex + 1].id as KanbanColumnId);
          }
        }
      }
  ...
  return (
    <BaseModal isOpen={isOpen} onClose={onClose} maxWidth="max-w-2xl">
      <div className="relative w-full max-w-2xl bg-card rounded-2xl overflow-hidden transform transition-all animate-in fade-in zoom-in duration-300 ease-in-out p-6">
        <div className="h-1 w-full bg-primary absolute top-0 left-0 transition-all"></div>
        <div className="flex flex-col max-h-[90vh]">
          {/* Header */}
          <div className="-b - flex items-center justify-between bg-card shrink-0 transition-colors p-6">
          ...
          <div className="p-6 overflow-y-auto custom-scrollbar text-foreground bg-card transition-colors">
            <div className="flex items-center gap-4 mb-6 p-4 bg-muted/50 rounded-2xl border border-border transition-colors">
            ...
            <form id="feedback-form" className="space-y-6" onSubmit={handleSubmit}>
              <div>
                ...
                <div className="flex gap-1 items-center">
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <i
                        className={`${rating >= star ? "fa-duotone fa-solid fa-star" : "fa-light fa-star"} text-[30px]`}
                      ></i>
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <textarea
                    className="w-full h-32 rounded-2xl border border-border bg-background text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 placeholder:text-muted-foreground resize-none p-3 transition-all duration-200 ease-in-out hover:border-ring"
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <label className={`relative flex flex-col items-center justify-center p-4 rounded-2xl border-2 cursor-pointer transition-all duration-200 ease-in-out  group ${recommendation === 'advance' ? 'border-primary bg-primary/5' : 'border-border bg-background hover:border-primary/50'}`}>
                    ...
                    <div className={`size-10 rounded-full flex items-center justify-center mb-2 transition-colors ${recommendation === 'advance' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground group-hover:bg-foreground/10 group-hover:text-foreground'}`}>
                      <Icon icon="material-symbols:thumb-up" aria-hidden="true" className={`h-5 w-5  text-2xl ${recommendation === 'advance' ? 'filled' : ''}`} />
                    </div>
                  ...
                  <label className={`relative flex flex-col items-center justify-center p-4 rounded-2xl border-2 cursor-pointer transition-all duration-200 ease-in-out  group ${recommendation === 'hold' ? 'border-foreground/50 bg-foreground/5' : 'border-border bg-background hover:border-foreground/30'}`}>
                    ...
                    <div className={`size-10 rounded-full flex items-center justify-center mb-2 transition-colors ${recommendation === 'hold' ? 'bg-foreground text-background' : 'bg-muted text-muted-foreground group-hover:bg-foreground/10 group-hover:text-foreground'}`}>
                      <Icon icon="material-symbols:pause" aria-hidden="true" className={`h-5 w-5  text-2xl ${recommendation === 'hold' ? 'filled' : ''}`} />
                    </div>
                  ...
                  <label className={`relative flex flex-col items-center justify-center p-4 rounded-2xl border-2 cursor-pointer transition-all duration-200 ease-in-out  group ${recommendation === 'reject' ? 'border-destructive bg-destructive/5' : 'border-border bg-background hover:border-destructive/30'}`}>
                    ...
                    <div className={`size-10 rounded-full flex items-center justify-center mb-2 transition-colors ${recommendation === 'reject' ? 'bg-destructive text-destructive-foreground' : 'bg-muted text-muted-foreground group-hover:bg-destructive/10 group-hover:text-destructive'}`}>
                      <Icon icon="material-symbols:thumb-down" aria-hidden="true" className={`h-5 w-5  text-2xl ${recommendation === 'reject' ? 'filled' : ''}`} />
                    </div>
              ...
              <div className="bg-muted/30 p-4 rounded-2xl border border-border transition-colors">
                <label className="block text-sm font-semibold mb-3 flex items-center gap-2 transition-colors">
                  <Icon icon="material-symbols:move-to-inbox" className="text-primary h-5 w-5" aria-hidden="true" />
          ...
          <div className="px-6 py-4 bg-muted/30 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4 shrink-0 transition-colors">
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              <button
                onClick={onClose}
                className="flex-1 sm:flex-none h-11 px-6 text-sm font-semibold text-foreground hover:bg-accent rounded-2xl border border-border bg-background transition-all duration-200 ease-in-out active:scale-95 outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
                Cancelar
              </button>
              <button
                type="submit"
                form="feedback-form"
                className="flex-1 sm:flex-none h-11 px-8 text-sm font-semibold text-primary-foreground bg-primary border border-border/40 hover:bg-primary/90 rounded-2xl transition-all duration-200 ease-in-out flex items-center justify-center gap-2 active:scale-95 outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
              >
                <Icon icon="material-symbols:save" className="text-lg h-5 w-5" aria-hidden="true" /> Confirmar
              </button>
            </div>
          </div>
        </div>
