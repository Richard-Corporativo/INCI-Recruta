...
const DroppableKanbanColumn: React.FC<DroppableKanbanColumnProps> = ({ id, title, dotColor, candidates, onCardClick, onQuickView, slaLimit, slaOwner }) => {
    ...
    return (
        <div
            ref={setNodeRef}
            className={`flex flex-col w-[300px] h-full rounded-2xl border shrink-0  transition-all duration-200 ease-in-out ${isOver ? 'bg-primary/5 border-primary ring-2 ring-primary/20' : 'bg-muted/30 border-border'
                }`}
        >
            ...
            <div className="p-2 flex-1 overflow-y-auto space-y-2 custom-scrollbar transition-colors min-h-[150px]">
                ...
                {isOver && candidates.length === 0 && (
                    <div className="h-20 border-2 border-dashed border-primary/30 rounded-2xl flex items-center justify-center text-xs text-primary font-medium animate-pulse">
                        Solte aqui
                    </div>
                )}
            </div>
