...
// --> otimizado: React.memo para evitar re-render de colunas não afetadas por drag-and-drop
const KanbanColumn: React.FC<KanbanColumnProps> = React.memo(({
    ...
    return (
        <div className="flex flex-col w-[300px] h-full bg-muted/30 rounded-lg border border-border shrink-0 shadow-sm">
            {/* Column Header */}
            <div className="p-4 border-b border-border flex justify-between items-center bg-card rounded-t-lg backdrop-blur-sm sticky top-0 z-10 h-14">
                <div className="flex items-center gap-2.5">
                    <span className={`size-2.5 rounded-full ${col.color.split(' ')[0]}`}></span>
                    <h3 className="font-bold text-sm text-foreground tracking-tight">{col.title}</h3>
                    <span className="bg-muted text-muted-foreground text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider border border-border">
                        {candidates.length}
                    </span>
                </div>
            </div>
